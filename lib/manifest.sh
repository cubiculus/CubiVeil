...
# ⚠️  ВНИМАНИЕ: Эти функции ДОЛЖНЫ быть определены в модулях!
# Если этот файл подключить после модуля install.sh, эти пустые заглушки
# перезапишут реальные функции модуля!
# 
# ПРАВИЛЬНЫЙ ПОРЯДОК ПОДКЛЮЧЕНИЯ:
#   1. Сначала подключите модуль: source "lib/modules/system/install.sh"
#   2. НЕ подключайте lib/manifest.sh в модульные скрипты!
#
# lib/manifest.sh используется только:
#   - В orchestrator.sh для управления ВСЕМИ модулями
#   - В тестах интеграции (где контролируется порядок подключения)

# Стандартный интерфейс модуля (должен быть определён в модуле)
# module_install() { :; }
# module_configure() { :; }
# module_enable() { :; }
# module_disable() { :; }
